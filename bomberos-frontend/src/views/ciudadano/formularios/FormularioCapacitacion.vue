<template>
  <form @submit.prevent="enviar" class="space-y-6">
    <!-- FASE 3c — FORM-DNB-005 (legacy CAPACI-JUR) -->
    <!-- Sección 1: Datos de la Entidad (formbns05: nombrerz, nit, oficina, legal) -->
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div class="md:col-span-2">
          ...
          <input
            ...
            maxlength="200"
            required
            placeholder="Ej: Empresa Minera S.A."
        ...
        <div>
          <label class="block text-xs font-semibold text-gray-700 mb-1">
            Entidad / Institución / Dependencia <span class="text-red-500">*</span>
          </label>
          <input
        ...
        <div class="md:col-span-2">
          <label class="block text-xs font-semibold text-gray-700 mb-1">
            Nombre del Responsable Legal / Representante <span class="text-red-500">*</span>
          </label>
          <input
            ...
            maxlength="200"
            required
            placeholder="Ej: Carlos Eduardo Morales"
            class="w-full px-3 py-2 rounded-lg bg-gray-50 border border-gray-200 text-sm focus:outline-none focus:bg-white focus:ring-2 focus:ring-dnb-primary/30 transition-all"
          />
        </div>
      </div>
    ...
    <!-- Sección 2: Datos del Solicitante / Contacto (formbns05: cedula, expedido, mail, telefono) -->
    <div class="bg-white rounded-xl shadow-sm p-5">
      <h3 class="text-sm font-bold text-gray-900 uppercase tracking-wider mb-4 flex items-center gap-2">
    ...
    <!-- Sección 3: Ubicación (formbns05: ciudad, departamento, provincia, municipio) -->
    <div class="bg-white rounded-xl shadow-sm p-5">
      <h3 class="text-sm font-bold text-gray-900 uppercase tracking-wider mb-4 flex items-center gap-2">
    ...
    <!-- Sección 4: Datos de la Capacitación (formbns05: capacitacion, cantidad) -->
    <div class="bg-white rounded-xl shadow-sm p-5">
      <h3 class="text-sm font-bold text-gray-900 uppercase tracking-wider mb-4 flex items-center gap-2">
        <span class="material-symbols-outlined text-[18px] text-dnb-primary">school</span>
        Datos del Curso de Capacitación
      </h3>

      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label class="block text-xs font-semibold text-gray-700 mb-1">
            Tipo de Capacitación Solicitada <span class="text-red-500">*</span>
          </label>
          <select
            v-model="form.tipo_capacitacion"
            required
            class="w-full px-3 py-2 rounded-lg bg-gray-50 border border-gray-200 text-sm focus:outline-none focus:bg-white focus:ring-2 focus:ring-dnb-primary/30 transition-all"
          >
            <option value="">Seleccione el curso...</option>
            <option v-for="t in TIPOS_CAPACITACION" :key="t" :value="t">{{ t }}</option>
          </select>
        </div>

        <div>
          <label class="block text-xs font-semibold text-gray-700 mb-1">
            Cantidad de Participantes <span class="text-red-500">*</span>
          </label>
          <input
            v-model.number="form.cantidad_participantes"
            type="number"
            min="1"
            max="1000"
            required
    ...
    <!-- Sección 5: Depósito Bancario (formbns05: deposito, fdeposito, docs) -->
    <div class="bg-white rounded-xl shadow-sm p-5">
      <h3 class="text-sm font-bold text-gray-900 uppercase tracking-wider mb-4 flex items-center gap-2">
    ...
    <!-- Sección 6: Observaciones (formbns05: observaciones) -->
    <div class="bg-white rounded-xl shadow-sm p-5">
      <h3 class="text-sm font-bold text-gray-900 uppercase tracking-wider mb-4 flex items-center gap-2">
...
<script setup>
// FASE 3c: Formulario de Capacitación SIPPCI (FORM-DNB-005 / legacy CAPACI-JUR)
// Catálogos hardcoded en el frontend — TODO FASE 5: API catalogos
...
/** Catálogo oficial de tipos de capacitación (legacy: tabla tcapacitacion) */
const TIPOS_CAPACITACION = [
  'Prevención y Protección Contra Incendios',
  'Manejo de Extintores',
  'Primeros Auxilios',
  'Trabajo en Altura',
  'Procedimientos en Evacuación',
]

/** Campos reales del legacy formbnd05s (normalizados a snake_case) */
const form = ref({
  nombre_razon_social: '',
  nit_ci: '',
  entidad_institucion: '',
...
const errores = computed(() => {
  ...
  if (!form.value.cantidad_participantes || form.value.cantidad_participantes < 1) {
    errs.push('Cantidad de participantes debe ser mayor a 0')
  }
  ...
  try {
    const payload = {
      ...
      datos_especificos: {
        ...
        tipo_capacitacion: form.value.tipo_capacitacion,
        cantidad_participantes: form.value.cantidad_participantes,
        // Sección 5: Depósito
        ...
        // Metadata legacy: identifica formulario fuente (formbns05)
        tipo_formulario: 'CAPACI-JUR',
